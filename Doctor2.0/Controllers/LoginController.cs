...
namespace Doctor2.Controllers
{
    public class LoginController : Controller
    {
        ...
        public IActionResult Login(LoginModelEntyties user)
        {
            ...
            //validamos que los campos no sean full o vacios antes de realizar la consulta
            if (user.UserName == "" || user.UserName == null)
            {
                ViewBag.ErrorUser = "Error: campo requerido";
                return View();
            }
            if (user.Password == "" || user.Password == null)
            {
                ViewBag.ErrorPass = "Error: campo requerido";
                return View();
            }
            ...
            try
            {
                ...
                if (login.Count > 0)
                {
                    foreach (var item in login)
                    {
                        ...
                        HttpContext.Session.SetInt32("IdDoctor", item.IdDoctor);
